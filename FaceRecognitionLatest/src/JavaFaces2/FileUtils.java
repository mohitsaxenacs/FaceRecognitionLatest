...
public class FileUtils
{
  ...
  private static final String TRAINING_DIR = "C:/Users/mosaxena/workspace/FaceRecognitionLatest/src/JavaFaces2/trainingImages/Device";
  ...
  // return all the names of the training image files + their paths
  {
	  ...
	  String[] fnms = null;
	  for(int i=1;i<=3;i++){
		  File dirF = new File(TRAINING_DIR.concat(String.valueOf(i)));
		     fnms = dirF.list( new FilenameFilter() {
		     ...
		     if(fnms != null && fnms.length != 0){
		    	    for (String fnm : fnms)
		    	      imFnms.add(TRAINING_DIR.concat(String.valueOf(i)) + File.separator + fnm);
		     }
		     
		     if (fnms == null) {
			      System.out.println(TRAINING_DIR.concat(String.valueOf(i)) + " not found");
			    }
			    else if (fnms.length == 0) {
			      System.out.println(TRAINING_DIR.concat(String.valueOf(i)) + " contains no " + " " + FILE_EXT + " files");
			    }
